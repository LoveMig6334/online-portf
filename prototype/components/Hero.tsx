...
const Hero: React.FC = () => {
  return (
    <section id="profile" className="relative pt-20 pb-12 overflow-hidden bg-gradient-to-b from-white to-surface-light">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        ...
        <div className="max-w-3xl space-y-6">
            ...
            <h1 className="text-4xl sm:text-6xl font-extrabold text-dark tracking-tight">
                Hi, I'm <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Nawat S.</span>
            </h1>
            <p className="text-lg sm:text-xl text-slate-500 font-medium leading-relaxed max-w-2xl mx-auto">
                Aspiring <span className="text-dark font-bold underline decoration-secondary decoration-4 underline-offset-2">Computer Engineer</span> & Student Council Member.
                Welcome to my personal portfolio showcasing my academic projects and achievements.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
                <a href="#portfolio" className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-8 py-3 rounded-full font-bold shadow-lg shadow-primary/25 transition-transform hover:-translate-y-1">
                ...
                <button className="flex items-center gap-2 bg-white text-dark border border-slate-200 hover:border-primary/50 px-8 py-3 rounded-full font-bold shadow-sm transition-colors">
                    <Download size={20} className="text-primary"/> Resume
                </button>
            </div>
        </div>
        ...
        <div className="mt-16 w-full max-w-4xl bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
             <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                 <div className="flex flex-col items-center">
                    ...
                    <span className="text-sm text-slate-500 font-medium uppercase tracking-wide mt-1">Projects</span>
                 </div>
                 <div className="flex flex-col items-center">
                    <span className="text-3xl font-bold text-dark">3.98</span>
                    <span className="text-sm text-slate-500 font-medium uppercase tracking-wide mt-1">GPA</span>
                 </div>
                 <div className="flex flex-col items-center">
                    <span className="text-3xl font-bold text-dark">M.4</span>
                    <span className="text-sm text-slate-500 font-medium uppercase tracking-wide mt-1">Grade</span>
                 </div>
                 <div className="flex flex-col items-center">
                    <span className="text-3xl font-bold text-dark">SC</span>
                    <span className="text-sm text-slate-500 font-medium uppercase tracking-wide mt-1">Council</span>
                 </div>
             </div>
        </div>
